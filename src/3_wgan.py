...
# Load the dataset
dataset = PhysicalActivityDataset('/home/pci/luka_bhi/dataset/combined_data.csv')
...
class Generator(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.model = nn.Sequential(
            ...
            nn.Tanh(),  # Use Tanh to restrict output between -1 and 1
        )
...
# Step 3: Training Setup without Gradient Penalty
...
real_corr = np.corrcoef(real_numerical_data.T)
syn_corr = np.corrcoef(synthetic_numerical_data.T)
pairwise_corr_dist = np.mean(np.abs(real_corr - syn_corr))
print(f"Pairwise correlation distance: {pairwise_corr_dist:.4f}")
